...
def get_frames(video_src: str) -> np.ndarray:
    ...
    После окончания видео продолжает отдавать последний кадр.
    """
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            print("End video, showing last frame...")
            # Если последний кадр есть, бесконечно его отдаём
...
def main(
    ...
    LPRnet.load_state_dict(
        torch.load(lpr_model_path, map_location=torch.device(DEVICE))
    )
