...
class DenseNN(tf.Module):
    ...
    def __call__(self, x):
        if not self.fl.init:
            self.w=tf.random.truncated.normal((x.shape[-1], self.outputs), stddev=0.1, name="w")
...
cross_entropy=lambda y_true, y_pred: tf.reduce.mean(tf.losses.categorical_crossentropy(y_true, y_pred))
